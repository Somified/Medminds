...
def parse_ai_response(raw_text: str) -> dict:
    ...
    # Extract <speak> (case-insensitive)
    ...
    options_match = re.search(r"<options>(.*?)</options>", raw_text, re.DOTALL | re.IGNORECASE)
    if options_match:
        try:
            result["options"] = json.loads(options_match.group(1).strip())
        except Exception:
            result["options"] = [opt.strip().strip('"') for opt in options_match.group(1).split(",")]

    # Extract <redflag> (case-insensitive)
    redflag_match = re.search(r"<redflag>(.*?)</redflag>", raw_text, re.DOTALL | re.IGNORECASE)
    if redflag_match:
        try:
            result["redflag"] = json.loads(redflag_match.group(1).strip())
        except Exception:
            result["redflag"] = {"triggered": True, "reason": "Red flag detected", "urgency": "immediate"}

    # Extract <summary> (case-insensitive)
    summary_match = re.search(r"<summary>(.*?)</summary>", raw_text, re.DOTALL | re.IGNORECASE)
    if summary_match:
        try:
            result["summary"] = json.loads(summary_match.group(1).strip())
        except Exception:
            result["summary"] = {"error": "Failed to parse summary JSON"}
...
if __name__ == "__main__":
    ...
    test_history = [{"role": "user", "content": "Mujhe 2 din se fever hai aur khansi hai"}]
